...
contract REVDeployerRegressions is TestBaseWorkflow {
    ...
    function test_noOOB_bothHooksPresent() public pure {
        ...
        JBPayHookSpecification[] memory specs = new JBPayHookSpecification[](arraySize);
        specs[0] = JBPayHookSpecification({hook: IJBPayHook(address(0xdead)), noop: false, amount: 1 ether, metadata: ""});
        specs[1] = JBPayHookSpecification({hook: IJBPayHook(address(0xbeef)), noop: false, amount: 2 ether, metadata: ""});
    }
