...
contract REVLoansRegressions is TestBaseWorkflow {
    ...
    //*********************************************************************//
    // ----------- setReferralProjectId: owner-settable referrer --------- //
    //*********************************************************************//

    /// @notice The constructor seeds `referralProjectId` with `(1, REV_ID)` so the default credit
    /// destination is the REV revnet on Ethereum mainnet, regardless of which chain the loan originates from.
    function test_referralProjectId_defaultIsMainnetREV() public view {
        // Packing per `JBMultiTerminal.currentReferralProjectId`: `(chainId << 48) | projectId`.
        uint256 expected = (uint256(1) << 48) | FEE_PROJECT_ID;
        assertEq(LOANS_CONTRACT.referralProjectId(), expected, "default packing must match (1, REV_ID)");
    }

    /// @notice Owner can repoint the referrer reference. Both fields encode losslessly into the storage slot.
    function test_setReferralProjectId_ownerCanRetarget() public {
        // Pick non-default values so the packing assertion isn't accidentally true.
        uint256 newChainId = 10;
        uint256 newProjectId = 42;
        uint256 expectedPacked = (newChainId << 48) | newProjectId;

        vm.expectEmit(true, true, false, true, address(LOANS_CONTRACT));
        emit IREVLoans.SetReferralProjectId({
            referralChainId: newChainId, referralProjectId: newProjectId, caller: address(this)
        });
        LOANS_CONTRACT.setReferralProjectId(newProjectId, newChainId);

        assertEq(
            LOANS_CONTRACT.referralProjectId(), expectedPacked, "stored slot must equal (chainId << 48) | projectId"
        );
    }

    /// @notice Non-owner cannot call the setter — gates fall back to `Ownable`'s revert.
    ...
    /// @notice A projectId outside the bottom 48 bits would bleed into the chainId field on pack — reject.
    function test_setReferralProjectId_revertsOnProjectIdOverflow() public {
        uint256 oversizedProjectId = uint256(type(uint48).max) + 1;
        vm.expectRevert(
            abi.encodeWithSelector(REVLoans.REVLoans_ReferralProjectIdTooLarge.selector, oversizedProjectId)
        );
        LOANS_CONTRACT.setReferralProjectId(oversizedProjectId, 1);
    }

    /// @notice A chainId outside the top 208 bits would silently drop bits on left-shift — reject.
    function test_setReferralProjectId_revertsOnChainIdOverflow() public {
        uint256 oversizedChainId = uint256(1) << 208;
        vm.expectRevert(abi.encodeWithSelector(REVLoans.REVLoans_ReferralChainIdTooLarge.selector, oversizedChainId));
        LOANS_CONTRACT.setReferralProjectId(1, oversizedChainId);
    }

    /// @notice Passing `(0, 0)` is a legitimate "disable referral credit" operation — accept it and store 0.
    function test_setReferralProjectId_zeroPairDisablesCredit() public {
        LOANS_CONTRACT.setReferralProjectId(0, 0);
        assertEq(LOANS_CONTRACT.referralProjectId(), 0, "zero pair must store 0");
    }
}
