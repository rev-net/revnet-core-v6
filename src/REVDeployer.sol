...
contract REVDeployer is ERC2771Context, IREVDeployer, IJBRulesetDataHook, IJBCashOutHook, IERC721Receiver {
        ...
        returns (bool)
    {
        ...
        return addr == loansOf[revnetId] || addr == address(buybackHook)
            || buybackHook.hasMintPermissionFor(revnetId, ruleset, addr)
        ...
        internal
    {
        ...
        if (buybackHookConfiguration.dataHook != IJBRulesetDataHook(address(0))) {
            ...
            if (buybackHookConfiguration.hookToConfigure != IJBBuybackHook(address(0))) {
                for (uint256 i; i < buybackHookConfiguration.poolConfigurations.length; i++) {
                    ...
                    // slither-disable-next-line unused-return
                    buybackHookConfiguration.hookToConfigure
                        .setPoolFor({
                            projectId: revnetId,
                            fee: poolConfig.fee,
        ...
        if (configuration.loans != address(0)) {
            _setPermission({
                operator: address(configuration.loans), revnetId: revnetId, permissionId: JBPermissionIds.USE_ALLOWANCE
            });
